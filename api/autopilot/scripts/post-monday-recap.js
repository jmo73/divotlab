...
const TRACKER_PATH = path_1.default.join(ROOT, 'lab-notes/lab-picks/season-tracker.json');
...
async function generateTweet(week, totals) {
...
async function main() {
    ...
    const tracker = JSON.parse(fs_1.default.readFileSync(TRACKER_PATH, 'utf8'));
    const week = tracker.weekly_picks[tracker.weekly_picks.length - 1];
    if (!week)
        throw new Error('No weekly_picks entries in season-tracker.json');
...
main().catch(async (err) => {
    ...
    await (0, publisher_1.tgNotify)(`❌ post-monday-recap failed:\n<code>${err.message}</code>`).catch(() => { });
    process.exit(1);
});
//# sourceMappingURL=post-monday-recap.js.map