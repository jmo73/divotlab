...
async function generateContent(eventName, course, targets, darkHorseName) {
    ...
    if (!parsed.tweet)
        throw new Error('Claude returned empty tweet');
    if (!parsed.ig_caption)
        throw new Error('Claude returned empty ig_caption');
...
async function main() {
    ...
    const statKeys = ['sg_ott', 'sg_app', 'sg_arg', 'sg_putt'];
    const weightMap = {
    ...
    const darkHorse = (rankedCandidates
    ...
    topThree.forEach((p, i) => console.log(`  ${i + 1}. ${p.name} — top10 ${(p.top10 * 100).toFixed(1)}% | fit ${Math.round(p.fit)}/100 | ${p.keyStrength}`));
    ...
    const pngBuf = await (0, imageGen_1.generateImage)('model-pick', (0, imageGen_1.modelPickFields)({
        eventName,
        conditionsSummary,
