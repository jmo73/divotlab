...
async function generateTweets(eventName, course, weights, top3, fieldSize) {
    const client = new sdk_1.default({ apiKey: process.env.ANTHROPIC_API_KEY });
    const wPct = (v) => Math.round(v * 100) + '%';
    const data = [
...
if (require.main === module) {
    main().catch(async (err) => {
        ...
        await (0, publisher_1.tgNotify)(`❌ post-tuesday-model failed:\n<code>${err.message}</code>`).catch(() => { });
        process.exit(1);
    });
}
//# sourceMappingURL=post-tuesday-model.js.map