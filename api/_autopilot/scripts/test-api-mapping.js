...
async function main() {
    ...
    const [liveData, inPlayResult, preTournData] = await Promise.all([
        (0, datagolf_1.getLiveTournamentStats)('event').catch(() => ({ eventName: 'No Tournament', players: [] })),
        (0, datagolf_1.getInPlayProbabilities)().catch(() => null),
        (0, datagolf_1.getPreTournamentPredictions)().catch(() => []),
    ]);
...
`);
    ...
    if (activePlayers.length > 0) {
        ...
        activePlayers.slice(0, 3).forEach((p, i) => {
            ...
            console.log(`       total:       ${p.total}  → formatScore: "${(0, imageGen_1.formatScore)(p.total)}"`);
            console.log(`       position:    "${p.position}"`);
            console.log(`       dg_rating:   ${p.dg_rating ?? 'N/A'}`);
            console.log(`       sg_total:    ${p.sg_total?.toFixed(2) ?? 'N/A'}  → formatSG: "${p.sg_total != null ? (0, imageGen_1.formatSG)(p.sg_total) : 'N/A'}"`);
        ...
        const { cutLine, bubblePlayers } = (0, datagolf_1.estimateCutLine)(activePlayers);
        console.log(`\n── Cut line estimate ─────────────────────────────────`);
        console.log(`   Estimated cut at: ${(0, imageGen_1.formatScore)(cutLine)} (top 65)`);
        console.log(`   Bubble players (within 1 shot): ${bubblePlayers.length}`);
        bubblePlayers.slice(0, 3).forEach(p => {
            console.log(`   · ${p.player_name} ${(0, imageGen_1.formatScore)(p.total)} (thru ${p.thru})`);
        });
    }
    else {
    ...
    const leaderFields = (0, imageGen_1.leaderboardFields)({
        ...
        insight: top5[0]
            ? `${top5[0].player_name} leads with ${(0, imageGen_1.formatSG)(top5[0].sg_total ?? 0)} SG: Total — model-implied win probability at ${inPlayMap.get(top5[0].dg_id)
                ? pct(inPlayMap.get(top5[0].dg_id).win)
                : '—'}.`
    ...
    const spotFields = (0, imageGen_1.spotlightFields)({
        ...
        stats: [
            { label: 'SG: App', value: leader?.sg_app != null ? (0, imageGen_1.formatSG)(leader.sg_app) : '+2.3' },
            { label: 'SG: Putt', value: leader?.sg_putt != null ? (0, imageGen_1.formatSG)(leader.sg_putt) : '+0.8' },
            { label: 'SG: OTT', value: leader?.sg_ott != null ? (0, imageGen_1.formatSG)(leader.sg_ott) : '+1.0' },
            { label: 'SG: ARG', value: leader?.sg_arg != null ? (0, imageGen_1.formatSG)(leader.sg_arg) : '+0.4' },
        ],
    ...
    const cpFields = (0, imageGen_1.courseProfileFields)({
    ...
    const wxFields = (0, imageGen_1.weatherFields)({
    ...
    const winFields = (0, imageGen_1.pickResultFields)({
    ...
    const mpFields = (0, imageGen_1.modelPicksFields)({
        eventName: activePlayers.length > 0 ? eventName : 'Travelers Championship',
        badge: 'Model Picks',
        picks: preTop3.map(p => ({
            name: p.player_name,
    ...
    const { cutLine, bubblePlayers: bubble } = activePlayers.length > 10
        ? (0, datagolf_1.estimateCutLine)(activePlayers)
    ...
    const caFields = (0, imageGen_1.cutAlertFields)({
        eventName: activePlayers.length > 0 ? eventName : 'Travelers Championship',
        cutLine: (0, imageGen_1.formatScore)(cutLine),
    ...
    for (const t of templates) {
        ...
        try {
            const buf = await (0, renderHtml_1.renderHtmlTemplate)(t.templateId, t.fields, { width: 1080, height: 1350 });
...
//# sourceMappingURL=test-api-mapping.js.map