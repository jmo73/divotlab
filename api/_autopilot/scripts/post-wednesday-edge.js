...
async function generateTweet(eventName, play) {
...
async function main() {
    ...
    const [preTour, top10Odds, cfData] = await Promise.all([
        ...
        (0, datagolf_1.getOutrightOdds)('top_10'),
    ...
    const candidates = preTour
        ...
        const bookPriority = ['draftkings', 'fanduel', 'bet365', 'caesars', 'betmgm'];
        let mktOdds = null;
        let book = '';
        ...
        if (edgePct <= 5)
            return null; // only surface meaningful edges (> 5%)
    ...
    if (play.fitScore)
        console.log(`  Course fit: ${play.fitScore}/100`);
...
if (require.main === module) {
    main().catch(async (err) => {
        ...
        await (0, publisher_1.tgNotify)(`❌ post-wednesday-edge failed:\n<code>${err.message}</code>`).catch(() => { });
        process.exit(1);
    });
}
//# sourceMappingURL=post-wednesday-edge.js.map