...
async function generateTweet(top3) {
...
async function main() {
    ...
    const pgaWithL12 = rankings.filter(p => (p.primary_tour ?? '').toLowerCase() === 'pga' &&
        p.sg_total != null &&
...
if (require.main === module) {
    main().catch(async (err) => {
        ...
        await (0, publisher_1.tgNotify)(`❌ post-evergreen-form-leaders failed:\n<code>${err.message}</code>`).catch(() => { });
        process.exit(1);
    });
}
//# sourceMappingURL=post-evergreen-form-leaders.js.map