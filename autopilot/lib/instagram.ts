...
export async function postToInstagram(
  ...
  const userId = process.env.INSTAGRAM_USER_ID ?? config.instagram.userId
  const token = process.env.INSTAGRAM_ACCESS_TOKEN ?? config.instagram.accessToken
  ...
  const mediaRes = await fetch(
    `${IG_API}/${published.id}?fields=permalink&access_token=${token}`
  )
  const media = (await mediaRes.json()) as { permalink?: string }
 ...
 */
export function checkTokenFreshness(): void {
