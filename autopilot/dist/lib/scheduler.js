...
let _previousLeaderboard = [];
let _previousRound = 0;
// Track which (player, round) combinations we've already fired for
const _moversAlreadyFired = new Set();
function hasMoverAlreadyFired(dgId, round) {
    return _moversAlreadyFired.has(`${dgId}:${round}`);
}
function markMoverFired(dgId, round) {
    _moversAlreadyFired.add(`${dgId}:${round}`);
}
// ─── Trigger eligibility checks ───────────────────────────────────────────────
async function isEligible(triggerType) {
    const isDup = await (0, db_1.checkDeduplication)(triggerType);
    return !isDup;
}
// ─── Data fetchers per trigger ────────────────────────────────────────────────
async function fetchLeaderboardData(status, round) {
    const [live, prePreds, rankings] = await Promise.all([
        (0, datagolf_1.getLiveTournamentStats)(),
        (0, datagolf_1.getPreTournamentPredictions)(),
        (0, datagolf_1.getRankings)(),
    ]);
    ...
    const topPredLive = topPredPlayer
        ...
        : null;
    const coords = (0, datagolf_1.lookupCourseCoords)(status.courseName || status.eventName);
    return {
...
async function fetchFinalData(status) {
    ...
    const winnerFinalPos = sorted.findIndex(p => p.player_name.toLowerCase() === topPick?.player_name.toLowerCase()) + 1;
    return {
        eventName: status.eventName,
        eventTier: status.eventTier,
        winner: winner
...
async function fetchMidRoundMoverData(status) {
    const live = await (0, datagolf_1.getLiveTournamentStats)();
    const players = live.players;
    ...
    const mover = (0, datagolf_1.detectMidRoundMover)(players, _previousLeaderboard, 5);
    _previousLeaderboard = players;
    if (!mover)
        return null;
    if (hasMoverAlreadyFired(mover.player.dg_id, round))
        return null;
...
async function fetchCutBubbleData(status) {
    const live = await (0, datagolf_1.getLiveTournamentStats)();
    const { cutLine, bubblePlayers } = (0, datagolf_1.estimateCutLine)(live.players);
    // Only fire if 5+ notable players on bubble
    if (bubblePlayers.length < 5)
        return null;
    const coords = (0, datagolf_1.lookupCourseCoords)(status.courseName || status.eventName);
    return {
        eventName: status.eventName,
...
async function fetchWeatherAngleData(status) {
...
async function fetchPreTournamentData(status) {
    const [prePreds, rankings, field] = await Promise.all([
        (0, datagolf_1.getPreTournamentPredictions)(),
        (0, datagolf_1.getRankings)(),
        (0, datagolf_1.getFieldUpdate)(),
    ]);
    ...
    const darkHorse = fieldPreds
        .slice(10, 30)
        .sort((a, b) => (ratingMap.get(b.dg_id)?.dg_rating ?? 0) - (ratingMap.get(a.dg_id)?.dg_rating ?? 0))[0];
    const coords = (0, datagolf_1.lookupCourseCoords)(status.courseName || status.eventName);
    return {
        eventName: status.eventName,
        courseName: status.courseName,
...
async function fetchSleeperData(status) {
    const [live, prePreds, rankings] = await Promise.all([
        (0, datagolf_1.getLiveTournamentStats)(),
        (0, datagolf_1.getPreTournamentPredictions)(),
        (0, datagolf_1.getRankings)(),
...
async function fetchComparisonData(status) {
    const [live, rankings] = await Promise.all([
        (0, datagolf_1.getLiveTournamentStats)(),
        (0, datagolf_1.getRankings)(),
    ]);
    const pair = (0, datagolf_1.selectComparisonPair)(live.players, rankings);
    if (!pair)
        return null;
    const [pA, pB] = pair;
    const sorted = [...live.players].sort((a, b) => a.total - b.total);
...
// ─── Main scheduler ───────────────────────────────────────────────────────────
async function runScheduler(jobType) {
    ...
    const status = await (0, datagolf_1.getTournamentStatus)();
    if (status.state === 'OFF') {
        return null;
    }
    ...
    for (const candidate of candidates) {
        ...
        if (!data)
            continue; // fetch returned null (e.g. no mover found, no bubble players)
        return {
...
async function runEvergreenScheduler() {
    for (const triggerType of EVERGREEN_PRIORITY) {
        const dedupPassed = await isEligible(triggerType);
        if (!dedupPassed)
            continue;
        const item = await (0, db_1.getNextEvergreenItem)(triggerType);
        if (!item)
            continue;
        return {
            triggerType,
