...
exports.regenerateCaption = regenerateCaption;
exports.generateEvergreenCaptions = generateEvergreenCaptions;
const sdk_1 = __importDefault(require("@anthropic-ai/sdk"));
const config_1 = require("./config");
const enrichment_1 = require("./enrichment");
const client = new sdk_1.default({ apiKey: config_1.config.anthropic.apiKey });
...
function getEventHashtag(eventName) {
    return eventName.replace(/[^a-zA-Z0-9]/g, '');
}
function xHashtags(eventName) {
    return `#PGATour #${getEventHashtag(eventName)} #GolfAnalytics`;
}
function igHashtags(eventName) {
...
function buildUserPrompt(req) {
    ...
    const contextSummary = (0, enrichment_1.buildContextSummary)(context);
    ...
    switch (triggerType) {
        ...
        case 'live_leaderboard_r3_end': {
            ...
            const leaderboard = (d.top5 ?? []).map((p, i) => `${i + 1}. ${p.playerName}: ${formatScore(p.score)} | DG Rating: ${p.dg_rating ?? '—'} (${p.dgRatingPercentile ?? '—'}th pct) | SG Total: ${p.sg_total != null ? formatSG(p.sg_total) : '—'}`).join('\n');
...
Include "via @DataGolf" at the end.`;
        ...
        case 'cut_bubble_alert': {
            const d = data;
            const playerList = d.players.slice(0, 5).map(p => `${p.playerName}: ${formatScore(p.score)} (thru ${p.holesPlayed})`).join('\n');
...
${footer}`;
        ...
        case 'weather_angle': {
            const d = data;
            const windPlayers = (d.playersWhoExcelInWind ?? []).map(p => `${p.playerName}: ${p.windConditionRecord}`).join('\n');
...
${footer}`;
        ...
        case 'pre_tournament_model_picks': {
            const d = data;
            const picks = d.picks.slice(0, 3).map((p, i) => `${i + 1}. ${p.playerName} — ${p.winProbability}% win probability, Course Fit: ${p.courseFitScore}/100\n   Key strength: ${p.keyStrength}`).join('\n');
 ...
 */
async function generateCaptions(triggerType, context, data) {
 ...
 */
async function regenerateCaption(params) {
 ...
 */
async function generateEvergreenCaptions(params) {
...
// ─── Fallback captions ────────────────────────────────────────────────────────
function buildFallbackCaption(triggerType, platform, data) {
    const p1 = data.top5?.[0];
    const eventName = data.eventName ?? 'this week\'s event';
