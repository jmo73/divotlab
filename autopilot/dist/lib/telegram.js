...
exports.sendApprovalMessage = sendApprovalMessage;
exports.sendEditPrompt = sendEditPrompt;
exports.sendUpdatedPreview = sendUpdatedPreview;
exports.sendPostConfirmation = sendPostConfirmation;
exports.sendExpiryNotice = sendExpiryNotice;
const config_1 = require("./config");
const TELEGRAM_API = `https://api.telegram.org/bot${config_1.config.telegram.botToken}`;
const CHAT_ID = config_1.config.telegram.chatId;
...
function buildCancelKeyboard(postId) {
    return [[{ text: 'Cancel', callback_data: `cancel:${postId}` }]];
}
// ─── Message text builders ────────────────────────────────────────────────────
function buildApprovalImageCaption(post) {
...
function buildCaptionPreviewText(post) {
...
function buildEditPromptText(post, platform) {
...
function buildUpdatedPreviewText(post, platform) {
    const platformLabel = platform === 'twitter' ? 'X' : platform === 'instagram' ? 'Instagram' : 'both platforms';
...
async function editTelegramMessage(messageId, text, inlineKeyboard) {
    const body = {
        chat_id: CHAT_ID,
 ...
 */
async function sendApprovalMessage(post, imageBuffer) {
    ...
    const textMsg = await sendTelegramMessage(buildCaptionPreviewText(post), buildApprovalKeyboard(post.id));
    return textMsg.message_id;
}
// ─── Edit flow ────────────────────────────────────────────────────────────────
async function sendEditPrompt(post, platform) {
    const msg = await sendTelegramMessage(buildEditPromptText(post, platform), buildCancelKeyboard(post.id));
    return msg.message_id;
}
async function sendUpdatedPreview(post, platform) {
    const msg = await sendTelegramMessage(buildUpdatedPreviewText(post, platform), buildApprovalKeyboard(post.id));
    return msg.message_id;
}
// ─── Post confirmation ────────────────────────────────────────────────────────
async function sendPostConfirmation(post, twitterResult, instagramResult) {
    const lines = [];
...
async function sendExpiryNotice(triggerLabel) {
