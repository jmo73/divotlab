...
import { getCourseFit } from '../lib/datagolf'
...
async function generateTweet(
  eventName:     string,
  weights:       { app: number; putt: number; ott: number; arg: number },
  leaders: {
    app:  { name: string; val: string; fitRank: number }
    putt: { name: string; val: string; fitRank: number }
    ott:  { name: string; val: string; fitRank: number }
    arg:  { name: string; val: string; fitRank: number }
  }
): Promise<string> {
...
async function main() {
  ...
  // Find field leader for each SG category using the data already in cfData.field
  type SgKey = 'sg_app' | 'sg_putt' | 'sg_ott' | 'sg_arg'
  const keys: SgKey[] = ['sg_app', 'sg_putt', 'sg_ott', 'sg_arg']

  function fieldLeader(key: SgKey) {
...
if (require.main === module) {
  main().catch(async err => {
    ...
    await tgNotify(`❌ post-thursday-sg-leaders failed:\n<code>${(err as Error).message}</code>`).catch(() => {})
    process.exit(1)
  })
}
