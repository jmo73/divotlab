...
import { getCourseFit, getRankings, formatPlayerName } from '../lib/datagolf'
...
async function generateTweet(
  ...
  dominantPct:   number,
  riser: { name: string; l12: string; l24: string; delta: string; fitRank: number }
): Promise<string> {
...
async function main() {
  ...
  const [cfData, rankings] = await Promise.all([getCourseFit(), getRankings()])
  ...
  const l12Map: Record<StatKey, keyof typeof rankings[0]> = {
    sg_app: 'sg_app_l12', sg_putt: 'sg_putt_l12', sg_ott: 'sg_ott_l12', sg_arg: 'sg_arg_l12',
  }
  const l24Map: Record<StatKey, keyof typeof rankings[0]> = {
    sg_app: 'sg_app_l24', sg_putt: 'sg_putt_l24', sg_ott: 'sg_ott_l24', sg_arg: 'sg_arg_l24',
  }
  const dominantKey = (Object.keys(weightMap) as StatKey[]).sort((a, b) => weightMap[b] - weightMap[a])[0]
  ...
  const candidates = cfData.field
    .map(fp => {
      ...
      if (!r) return null
      const l12 = r[l12Map[dominantKey]] as number | undefined
      const l24 = r[l24Map[dominantKey]] as number | undefined
      if (l12 == null || l24 == null) return null
      const delta = l12 - l24
      return {
        name:    formatPlayerName(r.player_name),
        l12,
        l24,
        delta,
        fitRank: fp.rank,
    ...
    .filter((x): x is NonNullable<typeof x> => x !== null && x.delta > 0.15)
...
if (require.main === module) {
  main().catch(async err => {
    ...
    await tgNotify(`❌ post-monday-form-check failed:\n<code>${(err as Error).message}</code>`).catch(() => {})
    process.exit(1)
  })
}
