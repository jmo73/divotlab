...
async function generateTweets(
  ...
  dominantPct: number,
  courseNotes: string,
  leader: { name: string; statVal: string; winPct: string; fitScore: string },
  secondPct: number,
  secondLabel: string
): Promise<{ tweetCourse: string; tweetLeader: string }> {
  ...
  const parsed = JSON.parse(match[0]) as { tweet_course?: string; tweet_leader?: string }
  return {
...
async function main() {
  ...
  const notes     = (weights as unknown as { notes?: string }).notes ?? ''
  ...
  console.log('\nGenerating tweets...')
  const { tweetCourse, tweetLeader } = await generateTweets(
    eventName, course, dominantLabel, dominantPct, notes,
    leader, secondPct, secondLabel
  )
...
if (require.main === module) {
  main().catch(async err => {
    ...
    await tgNotify(`❌ post-thursday-course-stat failed:\n<code>${(err as Error).message}</code>`).catch(() => {})
    process.exit(1)
  })
}
