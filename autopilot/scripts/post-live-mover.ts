...
async function main() {
  ...
  const { players } = await getLiveTournamentStats(String(round) as '1' | '2' | '3' | '4')
  ...
  if (leader.winPct) console.log(`  Win prob: ${leader.winPct}`)
  if (cfPlayer)      console.log(`  Course fit: ${Math.round(cfPlayer.fitScore)}/100`)
  if (isAlsoTotalLeader) console.log('  Also leads SG: Total this round')
...
if (require.main === module) {
  main().catch(async err => {
    ...
    await tgNotify(`❌ post-live-mover failed:\n<code>${(err as Error).message}</code>`).catch(() => {})
    process.exit(1)
  })
}
