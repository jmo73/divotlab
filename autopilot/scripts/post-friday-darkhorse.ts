...
async function generateTweet(
  eventName: string,
  pick: { name: string; fitScore: string; fitRank: string; winRank: string; winPct: string; dominantStat: string; dominantVal: string }
): Promise<string> {
...
if (require.main === module) {
  main().catch(async err => {
    ...
    await tgNotify(`❌ post-friday-darkhorse failed:\n<code>${(err as Error).message}</code>`).catch(() => {})
    process.exit(1)
  })
}
